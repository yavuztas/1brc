...
if [ ! -f target/CalculateAverage_yavuztas_image ]; then
    NATIVE_IMAGE_OPTS="--initialize-at-build-time=dev.morling.onebrc.CalculateAverage_yavuztas\$MultiRegionProcessor --gc=epsilon -O3 -march=native -R:MaxHeapSize=128m -H:TuneInlinerExploration=1 --enable-preview"
